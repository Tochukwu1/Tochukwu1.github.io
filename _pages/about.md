---
permalink: /
title: "Tochukwu Elijah Ogri"
excerpt: "About me"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

I am a PhD student and Graduate Research Assistant in [SCC Lab](https://scc-lab.github.io/) at the [University of Florida](https://ufl.edu), advised by Dr. [Rushikesh Kamalapurkar](https://mae.ufl.edu/people/name/rushikesh-kamalapurkar/). My research focuses on using control theory to develop stability-aware reinforcement learning techniques for safety-critical autonomous systems. I also work on learning-based control methods for robotic systems exhibiting nonsmooth phenomena, where policy synthesis is particularly challenging. Additionally, I have completed multiple [internships](https://tochukwu1.github.io/industry_experiences/) at the Autonomous Vehicles Lab (UF REEF) supporting U.S. Air Force Research Laboratory initiatives.

Previously, I earned my B.Eng. in Mechanical Engineering (First Class Honors) from the University of Benin in December 2019. From January 2022 to August 2024, I completed Ph.D. coursework and served as a Graduate Research Assistant at Oklahoma State University before transferring to the University of Florida under the same advisor. Prior to my graduate studies, I worked as a Software Engineer Graduate Trainee at Ecobank Transnational Incorporated.

Read about my [Projects](https://tochukwu1.github.io/projects/), [Industry Experiences](https://tochukwu1.github.io/industry_experiences/), and [Publications](https://tochukwu1.github.io/publications/).

## Research interests

- Adaptive optimal control and model-based reinforcement learning  
- Safety-critical controllers using Control Barrier Functions (CBFs)  
- State and parameter estimation  
- Real-time obstacle avoidance and motion planning  
- Sparsity-promoting system identification  
- Autonomous satellite inspection and Guidance, Navigation, and Control (GNC) architectures  
- Vision-based motion planning and monocular camera integration  


## Experiences

* **[Autonomous Vehicles Lab, UF REEF](https://avl.reef.ufl.edu/) – Robotics & Autonomy Intern** (May 2026 – July 202; May 2024 – Jul. 2024; May 2023 – Aug. 2023)  
* **[Systems, Cognition, and Control (SCC) Lab, UF](https://scc-lab.github.io/) – Graduate Research Assistant** (Aug. 2024 – Present)  
* **[Systems, Cognition, and Control (SCC) Lab, OSU](https://scc-lab.github.io/)  – Graduate Research Assistant** (Jan. 2022 – Aug. 2024)  
* **Ecobank Transnational Incorporated – Software Engineer Graduate Trainee** (May 2021 – Dec. 2021)

For more details, see my [resume](https://tochukwu1.github.io/cv/).